...
def utm_list():
    ...
    '''
    vms = [{"uuid": "1098", "state": "poweroff", "name": "deb13"}]
    return vms
...
# ---------------------------------------------------------
# GUI mode with menu bar + working shortcuts
# ---------------------------------------------------------
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # -------------------------------------------------
        # Menu shows CTRL shortcuts (cross-platform)
        # -------------------------------------------------
        ...
        # -------------------------------------------------
        # REAL SHORTCUTS (macOS uses Meta, others use Ctrl)
        # -------------------------------------------------
        if sys.platform == "darwin":
            # macOS: use Command (Meta)
            QShortcut(QKeySequence(Qt.META | Qt.Key_Equal), self, activated=self.console.zoom_in)
            QShortcut(QKeySequence(Qt.META | Qt.Key_Plus), self, activated=self.console.zoom_in)
            QShortcut(QKeySequence(Qt.META | Qt.Key_Minus), self, activated=self.console.zoom_out)
            QShortcut(QKeySequence(Qt.META | Qt.Key_0), self, activated=self.console.reset_zoom)
            QShortcut(QKeySequence(Qt.META | Qt.Key_L), self, activated=self.console.clear_console)
        else:
