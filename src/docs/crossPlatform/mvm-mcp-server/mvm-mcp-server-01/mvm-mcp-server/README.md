...
| Interface | Use case |
|-----------|----------|
| **MCP tools** (FastMCP) | LLMs – Claude Desktop, Cursor, etc. |
...
### MCP (stdio – recommended for Claude / Cursor)

```bash
python mvm_mcp_server.py
# or
python mvm_mcp_server.py --mode mcp --transport stdio
```
...
- `Hypervisor` – enum: `vmware | virtualbox | utm | hyperv`
- `VmIdentifier` – `hypervisor` + `vm`
- `StartVmRequest` – + optional `headless`
- `ResetVmRequest` – + optional `hard`
- `ActionResult`, `StatusResult`, `IpResult`, `ListResult` – typed responses

## MCP tools

Same operations as the REST surface, registered as FastMCP tools:
`list_vms`, `start_vm`, `stop_vm`, `pause_vm`, `unpause_vm`, `reset_vm`,
`get_vm_status`, `get_vm_ip`.

## Notes

- Business logic is shared between MCP tools and REST routes.
