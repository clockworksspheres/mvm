...
├── docker-compose.yml  # One-command REST stack
├── .dockerignore
├── requirements.txt
└── README.md
```
...
2. **mvm** on `PYTHONPATH` (or installed editable / baked into the image):
...
3. The target hypervisor process must already be **running** on the host for real VM control.
...
```bash
docker build -t mvm-mcp-server:latest .
docker run --rm -p 8000:8000 mvm-mcp-server:latest
```

### Use a local mvm checkout
...
### Important: host hypervisors

This container runs the **control plane** only. VirtualBox, VMware, UTM, and
Hyper-V live on the host. Full VM start/stop from inside Docker typically needs:

- `privileged: true` (and often `pid: host`)
- Bind-mounts of host tools (e.g. `/usr/bin/VBoxManage`) and device nodes

For day-to-day VM automation, prefer **native** install of this server on the
same machine as the hypervisor. Use Docker when you want a portable REST/MCP
endpoint for clients, CI, or remote agents.
...
Remote HTTP (e.g. Docker REST or MCP HTTP):
