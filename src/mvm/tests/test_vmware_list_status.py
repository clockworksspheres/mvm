...
import mvm.lib.vmware_list_status as vm   # <-- replace with actual filename


class TestVmFunctions(unittest.TestCase):

    @patch("mvm.lib.vmware_list_status.subprocess.check_output")
    ...
    @patch("mvm.lib.vmware_list_status.subprocess.check_output", side_effect=subprocess.CalledProcessError(1, "cmd", output=b"Error happened"))
    def test_run_vmrun_failure(self, mock_co):
        out = vm.run_vmrun(["vmrun", "list"])
        self.assertEqual(out, "Error happened")

    @patch("mvm.lib.vmware_list_status.run_vmrun")
    ...
    @patch("mvm.lib.vmware_list_status.RunWith")
    ...
    @patch("mvm.lib.vmware_list_status.Path")
    ...
    @patch("mvm.lib.vmware_list_status.Path.rglob")
    ...
    @patch("mvm.lib.vmware_list_status.find_all_vmx_files")
    @patch("mvm.lib.vmware_list_status.list_running_vms")
